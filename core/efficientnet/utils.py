...
class TpuBatchNormalization(tf.layers.BatchNormalization):
  ...
  def _cross_replica_average(self, t, num_shards_per_group):
    ...
    num_shards = 1 # tpu_function.get_tpu_context().number_of_shards
  ...
  def _moments(self, inputs, reduction_axes, keep_dims):
    ...
    num_shards = 1 # tpu_function.get_tpu_context().number_of_shards or 1
...
def drop_connect(inputs, is_training, drop_connect_rate):
  ...
  def dropout(inputs, drop_connect_rate):
    # Compute keep_prob
  ...
  # print('#######################################', inputs, drop_connect_rate)
  return tf.where(is_training, dropout(inputs, drop_connect_rate), inputs)
